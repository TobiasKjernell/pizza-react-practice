import { Link } from "react-router-dom";

const Button = ({ children, disabled, to }) => {
    const styles = `bg-yellow-400 uppercase font-semibold text-stone-800
          py-3 px-4 inline-block cursor-pointer tracking-wide rounded-full hover:bg-yellow-300
          transition-colors duration-200 focus:outline-none focus:bg-yellow-300 focus:ring focus:ring-yellow-300 focus:ring-offset-2 
          disabled:cursor-not-allowed sm:px-6 sm:py-4`

    if (to)
        return <Link className={styles} to={to}>{children}</Link>

    return (
        <button disabled={disabled} className={styles}>{children}</button>
    )
}

export default Button;